  ...
  <div class="flex flex-col gap-6 w-full max-w-2xl mx-auto">
    <div class="card bg-base-100 card-bordered shadow-lg text-center">
      <div class="card-body items-center">
        <h2 class="card-title text-3xl">Quiz Completed!</h2>
        ...
        <div class="alert shadow-sm mb-4" :class="accuracy >= 50 ? 'alert-success' : 'alert-warning'">
          <span>{{ feedbackMessage }}</span>
        </div>

        <div class="card-actions justify-center gap-4">
          <button class="btn btn-primary" @click="$emit('restart')">Try Again</button>
        </div>
      </div>
    </div>

    <div class="flex flex-col gap-4">
      <h3 class="text-xl font-bold px-2">Review Answers</h3>
      ...
      <div v-for="(res, index) in results" :key="index" 
        ...
        <div class="card-body">
          ...
          <p class="font-medium text-lg mt-2">{{ res.question_text }}</p>
