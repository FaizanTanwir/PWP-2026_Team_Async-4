    ...
    <div class="modal-box max-w-2xl border border-base-300 shadow-2xl">
      <form @submit.prevent="submitForm">
        ...
        <div class="space-y-4">
          ...
          <div v-if="!editingSentence" class="p-4 bg-primary/5 rounded-xl border border-primary/20 flex gap-3 items-end">
            ...
            <button type="button" @click="runAutomatedPreview" class="btn btn-primary" :disabled="previewLoading">
              <span v-if="previewLoading" class="loading loading-spinner loading-xs"></span>
              <Wand2 v-else class="size-4" /> Preview
            </button>
          </div>
          ...
          <div class="form-control w-full">
            ...
            <input v-model="form.text_target" type="text" class="input input-bordered w-full" required />
          </div>

          <!-- Source Language Input Block -->
          <div class="form-control w-full">
            <label class="label text-xs font-bold uppercase opacity-60">Source Text (Translation)</label>
            <input v-model="form.text_source" type="text" class="input input-bordered w-full" required />
          </div>

          <!-- Tokens Synchronization Dynamic Sub-Forms List -->
          <div class="divider text-xs uppercase opacity-50 font-bold tracking-widest">Tokenized Structural Words Map</div>
          
          <div class="max-h-56 overflow-y-auto space-y-2 pr-1">
            <div v-for="(word, idx) in form.words" :key="idx" class="grid grid-cols-3 gap-2 bg-base-200 p-2 rounded-lg relative items-center">
              <div>
                <input v-model="word.term" type="text" placeholder="Term" class="input input-sm input-bordered w-full" required />
              </div>
              <div>
                <input v-model="word.translation" type="text" placeholder="Translation" class="input input-sm input-bordered w-full" />
              </div>
              <div class="flex items-center gap-1">
                <input v-model="word.lemma" type="text" placeholder="Lemma (Base form)" class="input input-sm input-bordered w-full" />
                <button type="button" @click="removeWordToken(idx)" class="btn btn-square btn-ghost btn-xs text-error">
                  <X class="size-4" />
                </button>
              </div>
            </div>
          ...
          <button type="button" @click="addEmptyWordToken" class="btn btn-outline btn-xs gap-1 mt-2">
