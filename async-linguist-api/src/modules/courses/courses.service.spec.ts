...
describe('CoursesService', () => {
  ...
  describe('create', () => {
    it('should successfully create a course', async () => {
      ...
      const dto = { title: 'New Course', sourceLanguageId: 1, targetLanguageId: 2 };
  ...
  describe('update', () => {
    it('should update a course using preload', async () => {
      ...
      jest.spyOn(repo, 'preload').mockResolvedValue({ ...mockCourse, ...updateDto } as any);
      jest.spyOn(repo, 'save').mockResolvedValue({ ...mockCourse, ...updateDto } as any);
    ...
    it('should throw NotFoundException if preloading fails (record not found)', async () => {
      ...
      await expect(service.update(999, { title: 'Ghost' })).rejects.toThrow(NotFoundException);
    });
  });
